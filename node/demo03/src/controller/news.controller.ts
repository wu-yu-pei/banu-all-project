/*
 * @Author: wuyupei 495174699@qq.com
 * @Date: 2023-05-16 09:43:24
 * @LastEditors: wuyupei 495174699@qq.com
 * @LastEditTime: 2023-05-16 11:18:45
 * @FilePath: /banu-all-project/node/demo03/src/controller/news.controller.ts
...
class NewsController {
  ...
  async createNews(req: Request, res: Response) {
    ...
    if (!files) throw new BadRequest('请选择上传的文件');

    if (!NewsType[type]) throw new BadRequest('文章类型错误');

    await newsService.createNews(title, content, NewsType[type], files.file);
